...
const files: SuperCodeGeneratorFilesSchema = [
  ...
  // client function
  {
    ...
    template: ({ name, helpers }) => {
      ...
      export const use${pascalCase} = (props: PartialRequired<${pascalCase}Props>,) => {
        ...
        >({
          ...
          get: {
            fetcher: async () => {
              const { data } = await ${camelCase}Client(props as ${pascalCase}Props)
  ...
  {
    path: ({ name }) => {
      return `src/apiFunctions/${name}/ui/readme.md`
    },
    template: ({ name }) => {
      return `# ${name} UI
  ...
  {
    path: ({ name }) => {
      return `src/apiFunctions/${name}/utils/readme.md`
    },
    template: ({ name }) => {
      return `# ${name} Utils
      
      Add utils components here
      `
    },
  },
