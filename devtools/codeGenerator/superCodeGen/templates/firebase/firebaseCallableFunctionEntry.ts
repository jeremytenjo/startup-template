...
const files: SuperCodeGeneratorFilesSchema = [
  // Raw Function
  {
    ...
    template: ({ name, helpers }) => {
      ...
      export type ${pascalCase}Props<RouteSchema extends RouteSchemaProps = any> =
        ApiRouteProps<RouteSchema>
      
      export type ${pascalCase}Return<RouteSchema extends ApiRouteSchema> = RouteSchema['return']
      
      export default async function ${camelCase}<RouteSchema extends ApiRouteSchema>(
        ...
        throw new Error(${helpers?.wrapInTemplateLiteral({
          text: `Route doesn't exist - ${helpers?.wrapInTemplateLiteral({
            text: 'props.context.route',
            isPlaceholder: true,
          })}`,
        })})
      }
  ...
  // Client Function
  {
    ...
    template: ({ name, helpers }) => {
      ...
      return `import type { ${pascalCase}Return } from './${camelCase}.js'
      
      import {
        callFirebaseFunction,
        type ApiRouteSchema,
        type RouteSchemaProps,
        type ApiRouteClientProps,
      } from '@/firebaseFunctions/src/utils/useApiRouteData/useApiRouteData.js'
      
      export type ${pascalCase}ClientProps<RouteSchema extends RouteSchemaProps> =
        ApiRouteClientProps<RouteSchema>
      
      export default async function ${camelCase}Client<RouteSchema extends ApiRouteSchema>(
        props: ${pascalCase}ClientProps<RouteSchema>,
      ): ${pascalCase}ClientReturn<RouteSchema> {
      ...
      export type ${pascalCase}ClientReturn<RouteSchema extends ApiRouteSchema> = Promise<
...
export function get${pascalCase}DataId<RouteSchema extends ApiRouteSchema>(props: {
  ...
  const callId = ${helpers?.wrapInTemplateLiteral({
...
export default function use${pascalCase}Persist<RouteSchema extends ApiRouteSchema>(
  ...
  const ${camelCase} = useData<
    Awaited<${pascalCase}ClientReturn<RouteSchema>>['data'][0],
    ${pascalCase}ClientProps<RouteSchema>,
  ...
  // Routes Index
  {
    ...
    template: ({ name, helpers }) => {
      ...
      export default async function ${camelCase}Routes<
        RouteSchema extends ApiRouteSchema,
...
const template: SuperCodeGeneratorTemplateSchema = {
  type: 'Firebase Collable Function - Entry',
  files,
}
